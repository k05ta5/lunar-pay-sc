    ...
    crate::agreement_amount::AgreementAmountModule
{
    ...
    fn trigger_agreement_for_account(&self, agreement: Agreement<Self::Api>, account: ManagedAddress<Self::Api>) {
        ...
        let mut amount_to_charge: BigUint;

        match agreement.agreement_type {
            AgreementType::RecurringPayoutToSend => {
                self.trigger_agreement_for_recurring_payout_to_send(agreement, account)
            },
            AgreementType::RecurringPayoutToReceive => {
    ...
    fn trigger_agreement_for_recurring_payout_to_send(&self, agreement: Agreement<Self::Api>, account: ManagedAddress<Self::Api>) {
        let sender = agreement.creator;
        let receiver = account.clone();

        let amount_agreed: Amount<Self::Api> = self.get_amount_agreed_by_parties(agreement, account);
        let cycles = self.get_account_number_of_cycles_to_trigger(agreement.id, agreement.frequency, &account);

        let amount_per_cycle = amount_agreed.fixed_amount.unwrap();
        let total_amount = amount_per_cycle * cycles;

        if self.account_has_sufficient_balance(&sender, &agreement.token_identifier, &total_amount) {
            self.do_transfer_and_update_balance(&sender, &receiver, &agreement.token_identifier, &total_amount);
        }

        self.agreement_last_cycle_triggered_per_account(agreement.id, &account).set(
            self.compute_new_cycle_trigger_timestamp(agreement.id, &account, agreement.frequency, cycles)
        );
    }
}
